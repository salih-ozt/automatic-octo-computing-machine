...
// Upload/mesaj endpoint — önceden oluştur (handler içinde değil)
const postsPostRateLimit = rateLimit({ windowMs: 60 * 1000, max: 60 });
app.use('/api/posts', (req, res, next) => {
    if (req.method !== 'POST') return next();
    return postsPostRateLimit(req, res, next);
});
