    ...
    try {
        ...
        if (hasMedia) {
            ...
            if (isVideo) {
                ...
                try {
                    // ⚡ effort:2 → 5x daha hızlı encode (effort:4'ten)
                    await sharp(file.path, { sequentialRead: true })
        ...
        // ⚡ Hashtag'leri PARALEL işle (eskiden sıralıydı)
        if (content) {
            ...
            if (hashtagMatches) {
                ...
                await Promise.all(uniqueTags.map(async (cleanTag) => {
                    try {
                        // UPSERT ile tek sorgu - SELECT + INSERT/UPDATE yerine
                        const hId = uuidv4();
        ...
        // ⚡ Video varsa ARKA PLANDA işle - response BEKLEMEZ (ultra hızlı UX)
        if (req._pendingVideo) {
            const { videoId, tempPath } = req._pendingVideo;
            console.log(`🎬 Arka planda video başlatılıyor: ${videoId}`);
            // await YOK → kullanıcı anında 201 alır, video arka planda işlenir
        ...
        // ⚡ Bu kullanıcının feed cache'ini temizle (yeni gönderi var)
...
// ✅ HATA DÜZELTMESİ: Önceki kod sadece suspicious_login_reports tablosuna bakıyordu.
// forgot-password endpoint'i tokenı password_resets tablosuna kaydeder.
// Bu endpoint artık ÖNCE password_resets, SONRA suspicious_login_reports tablolarına bakar.
app.get('/api/auth/reset-password-direct', async (req, res) => {
    const { token } = req.query;
    if (!token) return res.status(400).send('<h2>Geçersiz bağlantı</h2>');
    try {
        // ✅ DÜZELTİLDİ: Önce password_resets tablosuna bak (forgot-password akışı)
        let record = await dbGet(
            `SELECT pr."userId", u.email FROM password_resets pr JOIN users u ON pr."userId"=u.id
             WHERE pr.token=$1 AND pr.used=FALSE AND pr."expiresAt">NOW() LIMIT 1`,
            [token]).catch(()=>null);
        ...
        if (!record) return res.send('<html><body style="font-family:sans-serif;text-align:center;padding:40px"><h2>❌ Bağlantı geçersiz veya süresi dolmuş</h2><p>Yeni bir şifre sıfırlama bağlantısı için uygulamayı kullanın.</p></body></html>');

        // Token geçerli — kullanıcıyı yeni şifre belirleme sayfasına yönlendir
        res.send(`<html>
<head><meta charset="UTF-8"><title>Şifre Sıfırlama - Agrolink</title>
<style>body{font-family:'Segoe UI',sans-serif;text-align:center;padding:60px;background:#f4f4f4}
.card{max-width:480px;margin:0 auto;background:#fff;border-radius:16px;padding:40px;box-shadow:0 4px 20px rgba(0,0,0,.1)}
h2{color:#2e7d32}p{color:#555}
.btn{display:inline-block;background:linear-gradient(135deg,#2e7d32,#4caf50);color:#fff;padding:16px 40px;border-radius:10px;text-decoration:none;font-weight:bold;font-size:16px;margin-top:20px}</style></head>
<body><div class="card">
<div style="font-size:64px">🔐</div>
<h2>Şifre Sıfırlama</h2>
<p><strong>${record.email}</strong> hesabı için şifre sıfırlama bağlantısı doğrulandı.</p>
<p>Yeni şifrenizi belirlemek için aşağıdaki butona tıklayın.</p>
<a href="/?resetToken=${encodeURIComponent(token)}&resetEmail=${encodeURIComponent(record.email)}" class="btn">🔑 Yeni Şifremi Belirle</a>
<p style="margin-top:30px;font-size:13px;color:#999">Bu bağlantı 10 dakika geçerlidir.</p>
</div></body></html>`);
