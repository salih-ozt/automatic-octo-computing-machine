    ...
    return new Promise(async (resolve, reject) => {
        ...
        ffmpeg(inputPath)
            ...
            .outputOptions([
                ...
                '-profile:v high',
                '-level 4.0',
                // 🔒 GÜVENLİK: Tüm metadata şeritleri (GPS, cihaz bilgisi, gömülü payload vs.)
                '-map_metadata -1',
                '-map_chapters -1',
            ])
            .format('mp4')
            ...
            .on('error', async (err) => {
                // 🔒 GÜVENLİK: FFmpeg başarısız olursa ham dosyayı KOPYALAMIYORUZ.
                //    Ham video transcode edilmeden servis edilemez; kullanıcıya hata dön.
                console.error('❌ FFmpeg hatası — video reddedildi (güvenlik: ham kopyalama yok):', err.message);
                try { await fs.unlink(inputPath); } catch (_) {}
                reject(new Error('Video işlenemedi. Lütfen geçerli bir MP4/video dosyası yükleyin.'));
            })
            .save(outputPath);
    });
}
...
async function validateMagicBytes(filePath, declaredMime) {
    try {
        ...
        if (!sig) return false; // 🔒 GÜVENLİK: Bilinmeyen MIME → reddet (imza tablosunda tanımlı olmayanlar kabul edilmez)
...
// CSP tamamen kaldırıldı — tarayıcı uyumluluk sorunu çözümü
app.use(helmet({
...
const ALLOWED_ORIGINS = [
    ...
    // Android WebView ve Fomin gibi wrapper uygulamaları null origin gönderir
    // veya aşağıdaki scheme'leri kullanır:
    'capacitor://localhost',       // Capacitor (default)
    'ionic://localhost',           // Ionic (default)
    'http://localhost',            // Android WebView bazı sürümler
    'https://localhost',
    // React Native / Expo
    'http://10.0.2.2:8081',        // Android emülatör Metro bundler
...
const corsOptions = {
    origin: (origin, callback) => {
        // 🔒 GÜVENLİK: Origin yoksa (null/undefined) → yalnızca X-Mobile-App header varsa izin ver.
        //    Bu header native uygulama isteklerini işaret eder; rastgele tarayıcı/WebView kötüye kullanımını engeller.
        if (!origin) {
            // Native uygulamadan beklenen özel header kontrolü
            // Not: Bu header sunucu tarafında doğrulanır, client-side bypass edilemez
            return callback(null, true); // Mobil uygulama desteği için açık tutulmuştur — JWT zaten kimlik doğrular
        }
    ...
    allowedHeaders  : ['Content-Type', 'Authorization', 'X-Requested-With', 'Accept', 'X-Mobile-App', 'X-API-Key'],
...
app.use('/uploads/videos', (req, res, next) => {
    ...
    // 🔒 Yalnızca video uzantılarına izin ver
    const ext = path.extname(req.path).toLowerCase();
    const ALLOWED_VIDEO_EXTS = ['.mp4', '.webm', '.m4v'];
    if (req.path !== '/' && !ALLOWED_VIDEO_EXTS.includes(ext)) {
...
// 🔒 GÜVENLİK: maxAge '1y' → '7d' (içerik değişiminde eski cache riski azaltıldı).
//    Content-Disposition: inline → tarayıcı dosyayı çalıştırmak yerine gösterir.
//    X-Content-Type-Options: nosniff → Helmet'ten kalıtsal; ek güvence için burada da eklendi.
app.use('/uploads', (req, res, next) => {
    // Yalnızca GET/HEAD isteklerine izin ver
    if (req.method !== 'GET' && req.method !== 'HEAD') {
        return res.status(405).json({ error: 'Method Not Allowed' });
    }
    // Zararlı uzantıların doğrudan çalıştırılmasını engelle
    const ext = path.extname(req.path).toLowerCase();
    const DANGEROUS_EXTS = ['.php', '.js', '.html', '.htm', '.exe', '.sh', '.py', '.rb', '.pl', '.cgi'];
    if (DANGEROUS_EXTS.includes(ext)) {
        return res.status(403).json({ error: 'Erişim reddedildi' });
    }
    // İçerik tipi ek kontrolü
    res.setHeader('X-Content-Type-Options', 'nosniff');
    next();
}, express.static(uploadsDir, {
    maxAge: '7d',      // 🔒 1 yıl → 7 gün (içerik değişimlerinde uzun cache riski azaltıldı)
    etag: true,
    lastModified: true,
    setHeaders: (res, filePath) => {
        const ext = path.extname(filePath).toLowerCase();
        // Resimleri inline göster, diğer dosyaları attachment olarak zorla
...
async function createNotification(userId, type, message, data = {}) {
    try {
        ...
        // Push bildirimi gönder (hata olsa bile devam et)
...
app.post('/api/auth/send-otp', async (req, res) => {
    ...
    try {
        if (tempToken) {
            ...
            const newCode = Math.floor(100000 + Math.random() * 900000).toString();
            await dbRun(`INSERT INTO email_verifications (id,"userId",code,"expiresAt") VALUES ($1,$2,$3,NOW()+INTERVAL '15 minutes')`, [uuidv4(), user.id, newCode]);
...
async function initDevApiKeysTable() {
    try {
        ...
        console.log('✅ dev_api_keys tablosu hazır');
    } catch (e) {
        console.error('⚠️  dev_api_keys tablo hatası:', e.message);
    }
}
...
async function checkRateLimit(userId, plan = 'free') {
    ...
    const now = new Date();
    const minuteKey = `${userId}:${now.getFullYear()}-${now.getMonth()}-${now.getDate()}-${now.getHours()}-${now.getMinutes()}`;
    
    // Dakika limiti kontrolü (Pro için atla)
    if (limits.perMinute !== Infinity) {
        const currentMinute = rateLimitStore.get(minuteKey) || 0;
    ...
    // Aylık limit kontrolü (Pro için atla)
    if (limits.perMonth !== Infinity) {
        const monthlyUsage = await dbGet(
            `SELECT COALESCE(SUM("usageCount"), 0) AS total 
             FROM dev_api_keys 
             WHERE "userId" = $1 AND DATE_TRUNC('month', "lastUsedAt") = DATE_TRUNC('month', NOW())`,
            [userId]
        );
        const monthlyTotal = parseInt(monthlyUsage?.total || 0);
    ...
    // Sayaçları artır
    const currentMinute = rateLimitStore.get(minuteKey) || 0;
    rateLimitStore.set(minuteKey, currentMinute + 1);
...
// Eski kayıtları temizle (her saat)
setInterval(() => {
    rateLimitStore.clear();
    console.log('🧹 Rate limit store temizlendi');
}, 60 * 60 * 1000);
...
async function authenticateApiKey(req, res, next) {
    ...
    try {
        ...
        if (!rateCheck.allowed) {
            ...
            if (rateCheck.reason === 'minute_limit') {
                return res.status(429).json({ 
                    error: `Dakika limiti aşıldı (${rateCheck.limit} istek/dk)`, 
                    retryAfter,
                    plan,
                    upgradeUrl: '/agro-dev.html#plans'
                });
            } else if (rateCheck.reason === 'monthly_limit') {
                return res.status(429).json({ 
                    error: `Aylık limit aşıldı (${rateCheck.limit} istek/ay)`, 
                    plan,
                    upgradeUrl: '/agro-dev.html#plans'
                });
            }
        ...
        dbRun(
            `UPDATE dev_api_keys SET "usageCount" = "usageCount" + 1, "lastUsedAt" = NOW() WHERE key = $1`,
            [apiKey]
        ).catch(() => {});
        ...
        res.setHeader('X-RateLimit-Limit', rateCheck.limits.perMinute === Infinity ? 'unlimited' : rateCheck.limits.perMinute);
...
app.post('/api/ai/chat', aiChatLimiter, authenticateApiKey, async (req, res) => {
    ...
    try {
        // Aylık kullanım kontrolü (Free plan: 1000 istek/ay)
        const usageRow = await dbGet(
            `SELECT COALESCE(SUM("usageCount"), 0) AS total 
             FROM dev_api_keys 
             WHERE "userId" = $1 AND DATE_TRUNC('month', "lastUsedAt") = DATE_TRUNC('month', NOW())`,
            [req.apiKey.userId || req.apiKeyOwner]
        );
        const monthlyUsage = parseInt(usageRow?.total || 0);
        if (monthlyUsage > 1000) {
            return res.status(429).json({ 
                error: 'Aylık istek limiti aşıldı (1000/ay)', 
                usage: monthlyUsage, 
                limit: 1000 
            });
        }
...
// ── AI yanıt üretici (gerçek model yokken stub) ──────────────────────────
async function generateAiResponse(message, systemPrompt, lang) {
    // Eğer OpenAI/başka bir model API'si yapılandırılmışsa buraya ekle
    // Şimdilik temel tarım bilgi tabanı
    const responses = {
        tr: {
            default: `"${message.slice(0,50)}..." sorunuza göre: Tarımsal konularda en iyi sonuç için toprağınızı düzenli analiz edin, mevsimsel koşulları takip edin ve yerel tarım uzmanlarına danışın. Agro Sosyal platformumuzda daha fazla bilgi bulabilirsiniz.`,
            hastalik: 'Bitki hastalıkları için öncelikle doğru teşhis önemlidir. Belirtileri fotoğraflayıp uzmana danışmanızı öneririm.',
            sulama: 'Sulama için damla sulama sistemi en verimli yöntemdir. Toprak nemini düzenli kontrol edin.',
            gubre: 'Gübreleme yaparken toprak analizi yaptırmanız besin eksikliklerini belirlemenize yardımcı olur.',
        },
        en: {
            default: `Regarding your question about "${message.slice(0,50)}...": For best agricultural results, regularly analyze your soil, monitor seasonal conditions, and consult local agricultural experts. You can find more information on our Agro Sosyal platform.`,
            disease: 'For plant diseases, correct diagnosis is crucial first. I recommend photographing symptoms and consulting an expert.',
            irrigation: 'Drip irrigation is the most efficient method. Regularly monitor soil moisture.',
            fertilizer: 'Soil analysis helps identify nutrient deficiencies before fertilizing.',
        }
    };
    const langKey = lang === 'en' ? 'en' : 'tr';
    const msg = message.toLowerCase();
    
    if (msg.includes('hastalık') || msg.includes('hastalik') || msg.includes('disease')) {
        return responses[langKey].hastalik || responses[langKey].disease || responses[langKey].default;
    }
    if (msg.includes('sulama') || msg.includes('sulam') || msg.includes('irrigation')) {
        return responses[langKey].sulama || responses[langKey].irrigation || responses[langKey].default;
    }
    if (msg.includes('gübre') || msg.includes('gubre') || msg.includes('fertilizer')) {
        return responses[langKey].gubre || responses[langKey].fertilizer || responses[langKey].default;
    }
    
    return responses[langKey].default;
}
