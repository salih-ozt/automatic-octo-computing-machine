...
// ─── GET /api/weather — BİRLEŞİK HAVA DURUMU ──────────────────
// Open-Meteo kullanır (tamamen ücretsiz, API key yok!)
// + Mevcut OpenWeatherMap varsa onu da dener (fallback)
app.get('/api/weather', authenticateToken, async (req, res) => {
    try {
        ...
        // WMO Weather Interpretation Codes → Türkçe + emoji
        const wmoDesc = (code) => {
            const map = {
                0: ['☀️','Açık ve güneşli'], 1: ['🌤️','Çoğunlukla açık'], 2: ['⛅','Parçalı bulutlu'],
                3: ['☁️','Kapalı'], 45: ['🌫️','Sisli'], 48: ['🌫️','Yoğun sis'],
                51: ['🌦️','Hafif çiseleyen'], 53: ['🌦️','Orta çiseleyen'], 55: ['🌧️','Yoğun çiseleyen'],
                61: ['🌧️','Hafif yağmurlu'], 63: ['🌧️','Orta yağmurlu'], 65: ['🌧️','Şiddetli yağmur'],
                71: ['❄️','Hafif kar'], 73: ['❄️','Orta kar'], 75: ['❄️','Yoğun kar'],
                77: ['🌨️','Kar taneleri'], 80: ['🌦️','Sağanak'], 81: ['🌧️','Kuvvetli sağanak'],
                82: ['⛈️','Şiddetli sağanak'], 85: ['❄️','Kar sağanağı'], 86: ['❄️','Yoğun kar sağanağı'],
                95: ['⛈️','Fırtına'], 96: ['⛈️','Fırtına ve dolu'], 99: ['⛈️','Şiddetli fırtına']
            };
            return map[code] || ['🌡️','Bilinmiyor'];
        };

        // ✅ Reverse geocode (Nominatim - ücretsiz)
        let cityName = '';
        try {
            ...
            if (geocodeRes.ok) {
                const gc = await geocodeRes.json();
                const a = gc.address || {};
                cityName = a.village || a.town || a.city || a.county || a.state || '';
                if (a.state && cityName !== a.state) cityName += ', ' + a.state;
            }
        } catch(_) {}

        // ✅ Open-Meteo API (tamamen ücretsiz, key yok)
        ...
        if (!meteoRes.ok) {
            const err = await meteoRes.text();
            return res.status(502).json({ error: 'Open-Meteo yanıt vermedi: ' + err.slice(0,100) });
        }
        ...
        const days = (daily.time || []).map((date, i) => {
            ...
            const [icon, desc] = wmoDesc(daily.weathercode?.[i] || 0);
            return {
                ...
                windMax: daily.windspeed_10m_max?.[i] ?? 0,
        ...
        const [curIcon, curDesc] = wmoDesc(cur.weathercode || 0);
        const temp = cur.temperature_2m ?? null;
        const precip = cur.precipitation ?? 0;
        const windSpeed = cur.windspeed_10m ?? 0;

        // ✅ TARIM TAKVİMİ — Hava bazlı öneriler
        const month = new Date().getMonth() + 1;
        const farmingCalendar = buildFarmingCalendar(temp, precip, windSpeed, days, month);

        // ✅ TARIM UYARILARI
        const alerts = buildWeatherAlerts(temp, precip, windSpeed, days);

        res.json({
            city: cityName || `${parseFloat(lat).toFixed(1)}°N, ${parseFloat(lon).toFixed(1)}°E`,
            current: {
                temp, feelsLike: cur.apparent_temperature ?? null,
                humidity: cur.relative_humidity_2m ?? null,
                windSpeed: Math.round(windSpeed),
                visibility: cur.visibility ? Math.round(cur.visibility / 1000) : null,
                precipitation: precip,
                description: curDesc, icon: curIcon,
...
function getTarimFiyatlari() {
    const base = [
        { id:'bugday',  ad:'Buğday',     emoji:'🌾', birim:'kg',  kategori:'Tahıllar',       featured:true,  fiyatBase:14.5 },
        { id:'misir',   ad:'Mısır',      emoji:'🌽', birim:'kg',  kategori:'Tahıllar',       featured:true,  fiyatBase:8.8 },
        { id:'arpa',    ad:'Arpa',       emoji:'🫘', birim:'kg',  kategori:'Tahıllar',       featured:false, fiyatBase:11.2 },
        { id:'findik',  ad:'Fındık',     emoji:'🌰', birim:'kg',  kategori:'Endüstriyel',    featured:true,  fiyatBase:185 },
        { id:'cay',     ad:'Çay',        emoji:'🍃', birim:'kg',  kategori:'Endüstriyel',    featured:false, fiyatBase:28 },
        { id:'pamuk',   ad:'Pamuk',      emoji:'☁️', birim:'kg',  kategori:'Endüstriyel',    featured:false, fiyatBase:22 },
        { id:'domates', ad:'Domates',    emoji:'🍅', birim:'kg',  kategori:'Sebze',          featured:true,  fiyatBase:12 },
        { id:'patates', ad:'Patates',    emoji:'🥔', birim:'kg',  kategori:'Sebze',          featured:false, fiyatBase:9.5 },
        { id:'biber',   ad:'Biber',      emoji:'🫑', birim:'kg',  kategori:'Sebze',          featured:false, fiyatBase:18 },
        { id:'sogan',   ad:'Soğan',      emoji:'🧅', birim:'kg',  kategori:'Sebze',          featured:false, fiyatBase:7.2 },
        { id:'elma',    ad:'Elma',       emoji:'🍎', birim:'kg',  kategori:'Meyve',          featured:false, fiyatBase:15 },
        { id:'uzum',    ad:'Üzüm',       emoji:'🍇', birim:'kg',  kategori:'Meyve',          featured:true,  fiyatBase:22 },
        { id:'kayisi',  ad:'Kayısı',     emoji:'🍑', birim:'kg',  kategori:'Meyve',          featured:false, fiyatBase:35 },
        { id:'aycicek', ad:'Ayçiçek',    emoji:'🌻', birim:'kg',  kategori:'Yağlı Tohumlar', featured:true,  fiyatBase:19 },
        { id:'soya',    ad:'Soya',       emoji:'🫛', birim:'kg',  kategori:'Yağlı Tohumlar', featured:false, fiyatBase:24 },
        { id:'kolza',   ad:'Kolza',      emoji:'🌼', birim:'kg',  kategori:'Yağlı Tohumlar', featured:false, fiyatBase:21 },
        { id:'sebzenin',ad:'Enginar',    emoji:'🥬', birim:'kg',  kategori:'Sebze',          featured:false, fiyatBase:45 },
    ];
    // Günlük rastgele değişim simüle et (gerçekte API'den gelir)
    const seed = Math.floor(Date.now() / (1000*60*60*6)); // 6 saatte bir değişir
    return base.map((u, i) => {
        const rng = Math.sin(seed * (i+1) * 7919) * 0.5 + 0.5;
        const degisim = parseFloat((rng * 4 - 2).toFixed(2)); // -2 ile +2 arası
        return { ...u, fiyat: parseFloat((u.fiyatBase + degisim).toFixed(2)), degisim };
    });
}
...
app.get('/api/tarim-fiyatlari', authenticateToken, async (req, res) => {
    try {
        const fiyatlar = getTarimFiyatlari();
        ...
        fiyatlar.forEach(f => { f.takipEdiliyor = takipSet.has(f.id); });
...
app.post('/api/hasat-takip/tarlalar/:id/foto', authenticateToken, upload.single('foto'), async (req, res) => {
    try {
        ...
        const dest = path.join(postsDir, filename);
        await sharp(req.file.path).resize(1080, 1080, { fit:'inside', withoutEnlargement:true }).webp({ quality:82 }).toFile(dest);
        await require('fs').promises.unlink(req.file.path).catch(()=>{});
