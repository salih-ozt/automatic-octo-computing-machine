...
app.use(helmet({
    // 🔒 CSP KAPALI - uygulama uyumluluğu için devre dışı
    ...
    // 🔒 XSS koruması aktif (eski tarayıcılar için)
    xssFilter: true,
    // 🔒 MIME sniffing engeli
    noSniff: true,
}));

// 🔒 XSS Koruması - tüm response'lara header ekle
...
// ============================================================
// 🔒 GİZLİLİK & 🔔 BİLDİRİM AYARLARI API ENDPOINTS
// ============================================================

// user_settings tablosunu oluştur (migration)
(async () => {
    try {
        await db.run(`
            CREATE TABLE IF NOT EXISTS user_settings (
                ...
                messageSetting TEXT DEFAULT 'everyone',
                hideLikes INTEGER DEFAULT 0,
                hideActivity INTEGER DEFAULT 0,
                hideReadReceipts INTEGER DEFAULT 0,
                -- Bildirim etkinlikleri
                notif_like INTEGER DEFAULT 1,
                notif_comment INTEGER DEFAULT 1,
                notif_mention INTEGER DEFAULT 1,
                notif_quote INTEGER DEFAULT 1,
                notif_repost INTEGER DEFAULT 1,
                notif_follow INTEGER DEFAULT 1,
                notif_follow_request INTEGER DEFAULT 1,
                notif_message INTEGER DEFAULT 1,
                notif_from_non_following INTEGER DEFAULT 1,
                notif_from_non_followers INTEGER DEFAULT 1,
                -- Sessiz saatler
        ...
        `);
        console.log('✅ user_settings tablosu hazır');
    } catch (err) {
        console.error('user_settings migration hatası:', err.message);
    }
})();

// Helper: kullanıcı ayarlarını getir veya oluştur
async function getUserSettings(userId) {
    let settings = await db.get('SELECT * FROM user_settings WHERE userId = ?', userId);
    if (!settings) {
        const id = require('uuid').v4();
        const now = new Date().toISOString();
        await db.run(
            `INSERT INTO user_settings (id, userId, createdAt, updatedAt) VALUES (?, ?, ?, ?)`,
            id, userId, now, now
        );
        settings = await db.get('SELECT * FROM user_settings WHERE userId = ?', userId);
    }
    return settings;
}

// GET /api/settings/privacy
app.get('/api/settings/privacy', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({
            settings: {
                ...
                messageSetting: settings.messageSetting || 'everyone',
        ...
        console.error('Privacy settings GET hatası:', error);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});
...
app.put('/api/settings/privacy', authenticateToken, async (req, res) => {
    try {
        ...
        const validVisibility = ['public', 'followers', 'approved'];
        const validMessage = ['everyone', 'followers', 'nobody'];
        if (!validVisibility.includes(profileVisibility)) {
            return res.status(400).json({ error: 'Geçersiz profileVisibility değeri' });
        }
        if (!validMessage.includes(messageSetting)) {
            return res.status(400).json({ error: 'Geçersiz messageSetting değeri' });
        }
        ...
        await db.run(
            ...
            hideLikes ? 1 : 0,
            hideActivity ? 1 : 0,
            hideReadReceipts ? 1 : 0,
            now,
            req.user.id
        );

        // users tablosundaki isPrivate sütununu da güncelle (uyumluluk)
        const isPrivateValue = (profileVisibility === 'approved' || privateAccount) ? 1 : 0;
        await db.run(
            'UPDATE users SET isPrivate = ?, updatedAt = ? WHERE id = ?',
            isPrivateValue, now, req.user.id
        );

        res.json({ success: true, message: 'Gizlilik ayarları güncellendi' });
    } catch (error) {
        console.error('Privacy settings PUT hatası:', error);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});
...
app.get('/api/settings/notifications', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({
            settings: {
                notif_like: settings.notif_like !== 0,
                notif_comment: settings.notif_comment !== 0,
                notif_mention: settings.notif_mention !== 0,
                notif_quote: settings.notif_quote !== 0,
                notif_repost: settings.notif_repost !== 0,
                notif_follow: settings.notif_follow !== 0,
                notif_follow_request: settings.notif_follow_request !== 0,
                notif_message: settings.notif_message !== 0,
                notif_from_non_following: settings.notif_from_non_following !== 0,
                notif_from_non_followers: settings.notif_from_non_followers !== 0,
        ...
        console.error('Notification settings GET hatası:', error);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});
...
app.put('/api/settings/notifications', authenticateToken, async (req, res) => {
    try {
        const {
            notif_like = true, notif_comment = true, notif_mention = true,
            notif_quote = true, notif_repost = true, notif_follow = true,
            notif_follow_request = true, notif_message = true,
            notif_from_non_following = true, notif_from_non_followers = true,
            quietHoursEnabled = false, quietStart = '22:00', quietEnd = '08:00',
        } = req.body;
        ...
        await db.run(
            `UPDATE user_settings SET
                notif_like = ?, notif_comment = ?, notif_mention = ?,
                notif_quote = ?, notif_repost = ?, notif_follow = ?,
                notif_follow_request = ?, notif_message = ?,
                notif_from_non_following = ?, notif_from_non_followers = ?,
                quietHoursEnabled = ?, quietStart = ?, quietEnd = ?,
                updatedAt = ?
             WHERE userId = ?`,
            notif_like ? 1 : 0, notif_comment ? 1 : 0, notif_mention ? 1 : 0,
            notif_quote ? 1 : 0, notif_repost ? 1 : 0, notif_follow ? 1 : 0,
            notif_follow_request ? 1 : 0, notif_message ? 1 : 0,
            notif_from_non_following ? 1 : 0, notif_from_non_followers ? 1 : 0,
            quietHoursEnabled ? 1 : 0, quietStart, quietEnd,
            now,
            req.user.id
        );
        ...
        console.error('Notification settings PUT hatası:', error);
        res.status(500).json({ error: 'Sunucu hatası' });
    }
});

console.log('🔒 Gizlilik & 🔔 Bildirim Ayarları API aktif!');
